...
// Pure color math (no React Native / Expo imports, so it's unit-testable and
// reusable). Given one or more brand seed colors it derives a coherent,
// WCAG-contrast-safe set: readable "on" colors for text/icons that sit on a
// brand color, plus pressed ("dark") and tinted ("soft") variants.
//
// Design stance: backgrounds/text/semantic colors stay neutral per light/dark
// mode (professional apps keep chrome neutral and apply the brand to accents,
// buttons, links, active states). This module only generates the brand-derived
// slots — never the neutrals — so injecting a brand can't wreck legibility.
...
export function generateBrandColors(
  ...
  const shade = (hex: string) => (mode === 'light' ? darken(hex, 0.16) : lighten(hex, 0.12));
  // Tinted background (badges, active rows): mostly the surface with a hint of brand.
  const soft = (hex: string) => mix(base.surface, hex, mode === 'light' ? 0.12 : 0.2);
