...
import { captureMessage, Sentry } from './sentry';
import { SENTRY_ENABLED } from '../config/sentry';
import { API_BASE_URL } from '../config/api';

const FEEDBACK_KEY = '@beta_feedback';

export async function submitFeedback({ description, whatYouWereDoing, reproSteps, currentScreen, lastCorrelationId }) {
  ...
  try {
    const existing = JSON.parse(await AsyncStorage.getItem(FEEDBACK_KEY) || '[]');
...
export async function getLocalFeedback() {
  try {
    return JSON.parse(await AsyncStorage.getItem(FEEDBACK_KEY) || '[]');
  } catch {
    return [];
  }
}
