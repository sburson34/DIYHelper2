...
// External integration clients (HTTP-based ones get typed HttpClients so retries/timeouts are uniform)
...
- repair_type: pick the single best category from the enumerated list. Use ""general"" if nothing fits.";
        ...
        try
        {
            ...
            try
            {
                ...
                // Fallback: if the model still emitted youtube_links as strings, harvest any raw queries from them
            ...
            // ── PubChem enrichment: surface hazard data for any recognized hazardous materials ──
            try
            {
...
app.MapGet("/api/weather", async ([FromQuery] string zip, [FromQuery] int? days, WeatherClient weather, ILogger<Program> logger) =>
{
...
app.MapGet("/api/reddit-discussions", async ([FromQuery] string query, RedditClient reddit, ILogger<Program> logger) =>
{
...
app.MapGet("/api/safety-data", async ([FromQuery] string chemical, PubChemClient pubChem, ILogger<Program> logger) =>
{
...
// ── Receipt OCR (Mindee) ───────────────────────────────────────────
app.MapPost("/api/receipt-ocr", async ([FromBody] ReceiptOcrRequest req, ReceiptOcrClient ocr, ILogger<Program> logger) =>
{
    if (!ocr.IsConfigured)
...
// ── Paint color match ──────────────────────────────────────────────
app.MapPost("/api/paint-color-match", ([FromBody] PaintColorRequest req, PaintColorClient paint, FeatureFlags features, ILogger<Program> logger) =>
{
    if (string.IsNullOrWhiteSpace(req.Base64Image))
        return Results.Json(new { error = "base64Image is required." }, statusCode: 400);
    byte[] data;
